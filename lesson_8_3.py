...
if __name__ == '__main__':
    my_list = []
    print('Вводите числа по одному для заполнения списка. Для выхода нажмите q')
    while True:
        user_itm = input('>>>')
        if user_itm == 'q':
            print(f'Программа завершена. Пользовательский список:\n{my_list}')
        try:
            if not is_number(user_itm):
                raise MyTypeError('В списке могут быть только числа!')
        except MyTypeError as err:
            print(err)
            continue
        my_list.append(user_itm)
